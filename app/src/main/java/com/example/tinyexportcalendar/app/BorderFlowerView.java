...
public class BorderFlowerView extends View {
    ...
    void setPaint(){
        dh = new DrawingHelper(maxx, maxy, x, dx, dy);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i<ta.length(); i++){
            canvas.drawPath(dh.generateComplexPath(dh.parseFloatArray(getResources().getStringArray(ta.getResourceId(i,i))), 1, 0), outlPaint);
        }
        ta = getResources().obtainTypedArray(R.array.leafs);
        for (int i = 0; i<ta.length(); i++){
            drawLeaf(canvas, dh.parseFloatArray(getResources().getStringArray(ta.getResourceId(i,i))));
        }
        ta = getResources().obtainTypedArray(R.array.lines);
        for (int i = 0; i<ta.length(); i++){
            canvas.drawPath(dh.generateLinesPath(dh.parseFloatArray(getResources().getStringArray(ta.getResourceId(i,i))), 1), outlPaint);
        }
        /*for (int fY : mFlowY) {
    ...
    void drawLeaf(Canvas canvas, float[] pathVals) {
        ...
        Path leafPath = dh.generateComplexPath(mPathVals, 1, 0);
        leafPath.setFillType(Path.FillType.WINDING);
        canvas.drawPath(leafPath,leafPaint);
        canvas.drawPath(dh.generateComplexPath(pathVals, 1, 0), outlPaint);
    }
